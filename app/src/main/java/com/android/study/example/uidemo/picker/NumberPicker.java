...
//    protected int mLineColor = Color.GRAY;

    protected Paint mLinePaint = null;

    private static final int MsgTouchUpChange = 100001;
    private int valueChangeIncrement = 0;
    private Handler mHandler = new Handler(){

        int preValue = valueChangeIncrement;
        ...
        public void handleMessage(Message msg) {
            ...
            if(msg.what == MsgTouchUpChange){

                if(preValue == valueChangeIncrement){
                    // 停止滑动了
                    valueChangeIncrement = 0;
                    if(mOnValueChangeListener != null){
                        mOnValueChangeListener.onLastValueChange(NumberPicker.this, mValue);
                    }
                }else{
                    mHandler.sendMessageDelayed(mHandler.obtainMessage(MsgTouchUpChange), 100);
                    preValue = valueChangeIncrement;
                }

                if(valueChangeIncrement >= MsgTouchUpChange){
                    valueChangeIncrement = 0;
                }
    ...
    public interface OnValueChangeListener {
        ...
        void onLastValueChange(NumberPicker picker, int newVal);
    }
    ...
    public NumberPicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        mVirtualButtonPressedDrawable = getResources().getDrawable(R.drawable.picker_virtual_button);
        ...
        mLabelPaint.setColor(getResources().getColor(R.color.picker_text_17));
    ...
    private boolean moveToFinalScrollerPosition(Scroller scroller) {
        ...
        if (overshootAdjustment != 0) {
            ...
            Log.i("lvjie", "moveToFinalScrollerPosition..."+amountToScroll);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                mHandler.sendMessage(mHandler.obtainMessage(MsgTouchUpChange));
    ...
    protected float measureLabelSize(String text){
       return mSelectorWheelPaint.measureText(text);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(!TextUtils.isEmpty(mUnit)){
            y = centerY + (TEXT_SIZE_MAX - TEXT_SIZE_MIN) / 2 + TEXT_MAX_ASCENT + TEXT_SIZE_LABEL;
            if(mUnitPos < mDisplayedMaxTextWidth){
                mUnitPos = mDisplayedMaxTextWidth;
            }
            canvas.drawText(mUnit, x + mUnitPos / 2, y, mLabelPaint);
        }

        Paint linePaint = mLinePaint;
        if(linePaint != null){
            float yy = centerY - mSelectorElementHeight * 0.5f;
            canvas.drawLine(0, yy, w, yy, linePaint);
            yy += mSelectorElementHeight;
            canvas.drawLine(0, yy, w, yy, linePaint);
        }
    }
     ...
     */
     private void changeValueByOne(boolean increment) {
        if (mHasSelectorWheel) {
            ...
            Log.i("lvjie", "changeValueByOne...increment="+increment);
            if (increment) {
    ...
    private void validateInputTextView(View v) {
        ...
        if (TextUtils.isEmpty(str)) {
            ...
            Log.i("lvjie", "validateInputTextView...");
    ...
    private void notifyChange(int previous, int current) {
        ...
        if(mSoundId != -1 && ts - mLastPlayTime >= 100) {
            mSoundPlayer.play(mSoundId, 1, 1, 0, 0, 1);
            mLastPlayTime = ts;
        }
        valueChangeIncrement++;
        if (mOnValueChangeListener != null) {
    ...
    class AccessibilityNodeProviderImpl extends AccessibilityNodeProvider {
        ...
        public boolean performAction(int virtualViewId, int action, Bundle arguments) {
            Log.i("lvjie","virtualViewId="+virtualViewId+"   action="+action);
            switch (virtualViewId) {
                case View.NO_ID: {
                    switch (action) {
